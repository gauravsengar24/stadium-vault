  ...
  return (
    <div className="space-y-6">
      ...
      <textarea
        ...
        maxLength={280}
        className="glass w-full rounded-2xl border border-glass-border px-4 py-3 text-sm outline-none transition placeholder:text-muted-foreground/50 focus:border-safety-red"
        rows={3}
      />
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
        {PANIC.map((p) => (
          <button
            ...
            onClick={() => trigger(p)}
            disabled={!!sending}
            className={`glass rounded-3xl p-6 text-left shadow-[0_20px_60px_-30px_oklch(0_0_0/0.6)] transition active:scale-[0.97] ${
              sending === p.key ? "opacity-50" : "hover:-translate-y-1 hover:border-safety-red/50"
            }`}
          >
